...
class Projection(utils.SaveLoad):
    def __init__(self, m, k, docs=None, use_svdlibc=False, power_iters=P2_EXTRA_ITERS, extra_dims=P2_EXTRA_DIMS):
        ...
        if docs is not None:
            ...
            self.u, self.s, self.d = None, None, None
...
class LsiModelAltered(interfaces.TransformationABC, basemodel.BaseTopicModel):
    """
...
#            self.print_debug(10)
    ...
    def __str__(self):
        return "LsiModelAltered(num_terms=%s, num_topics=%s, decay=%s, chunksize=%s)" % (
    ...
    def save(self, fname, *args, **kwargs):
        ...
        super(LsiModelAltered, self).save(fname, *args, ignore=['projection', 'dispatcher'], **kwargs)
    ...
    def load(cls, fname, *args, **kwargs):
        ...
        try:
            result.projection = super(LsiModelAltered, cls).load(projection_fname, *args, **kwargs)
        except Exception as e:
            logging.warning("failed to load projection from %s: %s" % (projection_fname, e))
        return result
#endclass LsiModelAltered
